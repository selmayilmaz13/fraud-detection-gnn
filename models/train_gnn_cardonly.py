"""
Trains a GraphSAGE model using card-only edges.
Transactions sharing the same card are connected as neighbors.
This is the simplest graph structure — baseline GNN.
...
3. Build transaction-transaction edges via shared card
4. Train GraphSAGE model
5. Evaluate with AUC-ROC, Recall, Precision
6. Save model to S3

Output: s3://fraud-detection-gnn/models/gnn_cardonly.pt
"""
...
def build_edges(data):
    print("Building transaction-transaction edges via card:")
...
def save_model_to_s3(model):
    ...
    torch.save(model.state_dict(), "/tmp/gnn_cardonly.pt")
    s3 = boto3.client("s3")
    s3.upload_file("/tmp/gnn_cardonly.pt", BUCKET_NAME, "models/gnn_cardonly.pt")
    print(f"  Saved to s3://{BUCKET_NAME}/models/gnn_cardonly.pt")
