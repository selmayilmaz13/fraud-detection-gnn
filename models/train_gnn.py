"""
Trains a Graph Neural Network (GraphSAGE) on the transaction graph
for fraud detection.
...
Output: s3://fraud-detection-gnn/models/gnn_model.pt
"""
...
def save_model_to_s3(model):
    print("Saving GNN model to S3:")
    torch.save(model.state_dict(), "/tmp/gnn_model.pt")
    s3 = boto3.client("s3")
    s3.upload_file("/tmp/gnn_model.pt", BUCKET_NAME, "models/gnn_model.pt")
    print(f"  Saved to s3://{BUCKET_NAME}/models/gnn_model.pt")

if __name__ == "__main__":
    data = load_graph()
    num_nodes = data["transaction"].x.shape[0]
    train_mask, test_mask = build_masks(num_nodes)
    model, x, y, tx_edge_index = train_model(data, train_mask, test_mask)
